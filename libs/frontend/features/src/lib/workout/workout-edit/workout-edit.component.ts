...
export class WorkoutEditComponent implements OnInit, OnDestroy {
  ...
  onSubmit(workoutInfo: IWorkoutInfo): void {
    ...
    if (this.workoutId) {
      this.sub?.add(this.workoutService.updateWorkout(this.workoutId, workout).subscribe(() => {
        this.router.navigate(['../../' + this.workoutId], { relativeTo: this.route });
      }))
    } else {
      this.sub?.add(this.workoutService.createWorkout({...workout, _id: null}).subscribe(() => {
